import { Goal, GoalLog, LeetcodeMetrics, GithubMetrics, FitnessMetrics } from "@prisma/client";
...
export class PlacementService {
   ...
   */
  static calculatePlacementReadiness(inputs: ScoringInputs): {
    ...
    logger.debug(`Scoring placement: Coding=${codingScore}, Project=${projectScore}, Fitness=${fitnessScore}, Final=${placementScore}`);
   ...
   */
  static calculateLifeBalance(inputs: ScoringInputs): {
    lifeBalanceScore: number;
    scores: { coding: number; projects: number; fitness: number; habits: number };
  } {
    ...
    logger.debug(`Scoring life balance: Avg=${avgScore}, Diff=${difference}, Penalty=${penalty}, Final=${lifeBalanceScore}`);
   ...
   */
  private static calculateGoalCompletionRate(categoryGoals: (Goal & { logs: GoalLog[] })[]): number {
    if (categoryGoals.length === 0) return 70; // baseline completion rate for unconfigured categories to avoid 0 score
    ...
    if (totalPossibleCompletions === 0) return 100;

...
export class GithubService {
   ...
   */
  static async fetchUserStats(username: string, token?: string): Promise<GithubStats> {
    try {
      if (!token) {
        logger.info(`No GitHub token for ${username}, using simulation`);
        return this.generateSimulatedStats(username);
      }

      logger.info(`Fetching high-fidelity GitHub metrics for: ${username} using token`);
      const headers = { 
      ...
      const reposRes = await axios.get("https://api.github.com/user/repos?per_page=15&sort=updated", { headers, timeout: 5000 });
      const repos = reposRes.data || [];

      const oneMonthAgo = new Date();
      oneMonthAgo.setDate(oneMonthAgo.getDate() - 30);
      const activeRepos = repos.filter((repo: any) => new Date(repo.pushed_at || repo.updated_at) > oneMonthAgo).length;
        ...
        repos.slice(0, 10).map(async (repo: any) => {
          try {
            const commitsRes = await axios.get(
              `https://api.github.com/repos/${repo.owner.login}/${repo.name}/commits?since=${sinceIso}&author=${actualUsername}&per_page=100`,
              { headers, timeout: 3000 }
            );
            commitsLast30Days += (commitsRes.data || []).length;
          } catch (err: any) {
            logger.warn(`Failed to fetch commits for ${repo.name}: ${err.message}`);
          }
        })
      );
      ...
      try {
        const prSearchRes = await axios.get(
          `https://api.github.com/search/issues?q=author:${actualUsername}+type:pr+created:>=${oneMonthAgo.toISOString().split("T")[0]}`,
          { headers, timeout: 4000 }
        );
        pullRequests = prSearchRes.data?.total_count || 0;
      } catch (err: any) {
        logger.warn(`Failed to search PRs for ${actualUsername}: ${err.message}`);
        pullRequests = 1;
      }
      ...
      logger.error(`GitHub real API failed for ${username}: ${error.message}. Response data: ${JSON.stringify(error.response?.data || {})}`);
      if (error.stack) {
   ...
   */
  private static generateSimulatedStats(username: string): GithubStats {
    let hash = 0;

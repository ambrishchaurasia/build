...
class AiController {
    ...
    static async chat(req, res, next) {
      ...
      `;
            if (openai) {
                logger_1.logger.info("Executing OpenAI chatbot session");
                const systemPrompt = `
        ...
        `;
                ...
                logger_1.logger.info("Executing local mock chatbot query");
     ...
     */
    static generateSimulatedReply(message, user, latestReport) {
      ...
      - If your projects are stagnant (commits: ${user.githubMetrics?.commitsLast30Days || 0} in 30 days), immediately open your IDE and start **${user.goals.find((g) => g.category === "PROJECT")?.title || "your primary project"}**.\n
2. **Weekly Strategy**:
   ...
   - Dedicate **20% to Fitness & Mind** (Meditation & pullups consistency) to avoid burnout.`;
            ...
            const ganeshGoal = user.goals.find((g) => g.title.toLowerCase().includes("ganesh"));
            const fapGoal = user.goals.find((g) => g.title.toLowerCase().includes("fap") || g.title.toLowerCase().includes("no fap"));

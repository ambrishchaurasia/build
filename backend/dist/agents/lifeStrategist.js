...
exports.LifeStrategist = void 0;
const openai_1 = __importDefault(require("openai"));
const logger_1 = require("../utils/logger");
const openai = process.env.OPENAI_API_KEY
    ? new openai_1.default({ apiKey: process.env.OPENAI_API_KEY })
    : null;
class LifeStrategist {
    static async synthesize(profile, codingInsight, projectInsight, scores, allGoalsCount) {
        try {
            if (openai) {
                logger_1.logger.info("Running Master Life Strategist via OpenAI API");
                const prompt = `
        ...
        `;
                ...
                logger_1.logger.info("Running simulated Life Strategist rule engine");
                return this.generateSimulatedInsight(profile, codingInsight, projectInsight, scores);
            }
        }
        catch (error) {
            logger_1.logger.error("Life Strategist synthesis failed, returning fallback", error);
            return {
    ...
    static generateSimulatedInsight(profile, codingInsight, projectInsight, scores) {
        ...
        if (codingInsight.strengths.length > 0)
            wins.push(codingInsight.strengths[0]);
        if (projectInsight.strengths.length > 0)
            wins.push(projectInsight.strengths[0]);
        ...
        if (codingInsight.weaknesses.length > 0)
            weaknesses.push(codingInsight.weaknesses[0]);
        if (projectInsight.weaknesses.length > 0)
            weaknesses.push(projectInsight.weaknesses[0]);
        // Streaks warning
...
exports.LifeStrategist = LifeStrategist;

...
const openai = process.env.OPENAI_API_KEY
    ...
    : null;
class FitnessCoach {
    static async analyze(metrics, goals) {
        try {
            ...
            if (openai) {
                logger_1.logger.info("Running AI Fitness Coach via OpenAI API");
                const prompt = `
        ...
        `;
                ...
                logger_1.logger.info("Running simulated Fitness Coach rule engine");
                return this.generateSimulatedInsight(metrics, goals);
            }
        }
        catch (error) {
            logger_1.logger.error("Fitness Coach analysis failed, returning baseline", error);
            return {
                score: 60,
    ...
    static generateSimulatedInsight(metrics, goals) {
        ...
        if (strengths.length === 0)
            strengths.push("Basic muscular recovery");
        if (weaknesses.length === 0)
            weaknesses.push("Sub-optimal core structural stability");
...
exports.FitnessCoach = FitnessCoach;

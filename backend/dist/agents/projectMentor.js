...
const openai = process.env.OPENAI_API_KEY
    ...
    : null;
class ProjectMentor {
    static async analyze(metrics, goals, careerGoal) {
        try {
            ...
            if (openai) {
                logger_1.logger.info("Running AI Project Mentor via OpenAI API");
                const prompt = `
        ...
        `;
                ...
                logger_1.logger.info("Running simulated Project Mentor rule engine");
                return this.generateSimulatedInsight(metrics, goals, careerGoal);
            }
        }
        catch (error) {
            logger_1.logger.error("Project Mentor analysis failed, returning baseline", error);
            return {
                score: 45,
    ...
    static generateSimulatedInsight(metrics, goals, careerGoal) {
        ...
        if (strengths.length === 0)
            strengths.push("Basic Git repository familiarity");
        if (weaknesses.length === 0)
            weaknesses.push("Project documentation and unit testing coverage");
...
exports.ProjectMentor = ProjectMentor;

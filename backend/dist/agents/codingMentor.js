...
class CodingMentor {
    static async analyze(metrics, goals, careerGoal) {
        try {
            ...
            if (openai) {
                logger_1.logger.info("Running AI Coding Mentor via OpenAI API");
                const prompt = `
        ...
        `;
                ...
                logger_1.logger.info("Running simulated Coding Mentor rule engine");
                return this.generateSimulatedInsight(metrics, goals, careerGoal);
            }
        }
        catch (error) {
            logger_1.logger.error("Coding Mentor analysis failed, returning baseline", error);
            return {
                score: 50,
    ...
    static generateSimulatedInsight(metrics, goals, careerGoal) {
        ...
        if (strengths.length === 0)
            strengths.push("Initiated problem-solving journey");
        if (weaknesses.length === 0)
            weaknesses.push("Competitive programming rating profile");
...
exports.CodingMentor = CodingMentor;
